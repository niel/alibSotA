...
function ShroudOnStart()
	...
	als.ui.onInit(function()
		...
		clock.timer = setInterval(1, function()
			...
			if clock.timeInGame then caption = caption.." | "..durToString(als.client.timeInGame) end
			if clock.timeInScene then caption = caption.." | "..durToString(als.scene.timeInScene) end

			setLabelCaption(clock.label, caption:style{ size=20, color="#c4a000" })
		end)

		als.ui.command.add("clock", function(source, action)
			local msgText ={
				"Clock deactivated",
			...
			elseif action == "tz" or action == "timezone" then
				als.ui.consoleLog("Your current timezone is: "..timezoneToString(clock.tz))
		...
		als.ui.shortcut.add("pressed", "RightAlt", "C", "clock")
